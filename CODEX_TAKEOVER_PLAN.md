...
- 以内容完整、信息架构、品牌表达、响应式可用性和核心交互为目标；参考旧站仅用于理解业务与视觉方向，不追求像素级复刻或逐像素截图一致。
...
6. 视觉调整以可读性、品牌一致性和跨尺寸可用性为验收标准；不将像素级差异作为阻塞条件。
...
- 执行链接、资源、安全和 SEO 检查。
- 对关键页面进行桌面端与移动端可用性和视觉一致性抽检，不要求像素级截图比对。
...
- 日期：2026-07-17
- 执行提交：`Fix reproducible Astro build environment`
...
- 日期：2026-07-17
- 执行提交：`Fix internal routes and asset integrity`
- 修改范围：首页及页脚的产品/新闻条目改为从内容集合选择，并在构建期校验所选条目存在；迁移内容中的失效产品、分类与新闻链接回退到对应索引页；新增构建产物完整性检查。
- 资源修复：补齐 About 页面缺失的 5 张本地背景图（`background-image2.jpg` 与 `bk-ground-1.jpg` 至 `bk-ground-4.jpg`）。
- 验证命令及结果：Node 22 下 `npm run build` 成功，生成 504 个静态页面；`npm run check:dist` 扫描 1,215 个产物文件，内部页面、`src`/`srcset`/内联背景资源、空链接、`javascript:` URL 及旧域名引用均为 0 项。
- 是否可以进入下一阶段：是
